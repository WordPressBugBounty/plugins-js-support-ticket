...
?>
<?php
$jsst_allPlugins = get_plugins(); // associative array of all installed plugins

$jsst_addon_array = array();
foreach ($jsst_allPlugins as $jsst_key => $jsst_value) {
    $jsst_addon_index = jssupportticketphplib::JSST_explode('/', $jsst_key);
    $jsst_addon_array[] = $jsst_addon_index[0];
}
?>
                    ...
                    <div id="jsst-lower-wrapper">
                        <div class="jsst-addon-installer-wrapper" >
                            <form id="jsticketfrom" action="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=premiumplugin&task=downloadandinstalladdons&action=jstask'),"download-and-install-addons")); ?>" method="post">
                                <div class="jsst-addon-installer-right-section-wrap step2" >
                                    ...
                                    <div class="jsst-addon-installer-right-addon-wrapper" >
                                        ...
                                        if(isset($_COOKIE['jsst_addon_install_data'])){
                                            $jsst_addon_install_data = jssupportticketphplib::JSST_safe_decoding(jssupportticket::JSST_sanitizeData(wp_unslash($_COOKIE['jsst_addon_install_data']))); // JSST_sanitizeData() function uses wordpress santize functions
                                        ...
                                        if($jsst_addon_install_data){
                                            $jsst_result = $jsst_addon_install_data;
                                            if(isset($jsst_result['status']) && $jsst_result['status'] == 1){
                                                if (isset($jsst_result['data']) && !empty($jsst_result['data'])) {
                                                    $jsst_addon_availble_count = 0;
                                                    foreach ($jsst_result['data'] as $jsst_key => $jsst_value) {
                                                        if (!in_array($jsst_key, $jsst_addon_array)) {
                                                            $jsst_addon_availble_count++;
                                                        }
                                                    }
                                                    if ($jsst_addon_availble_count == 0) {
                                                        $jsst_error_message = esc_html(__('All allowed add ons are already installed','js-support-ticket')) . '.';
                                                    }
                                                } else {
                                                    $jsst_error_message = esc_html(__('You are not allowed to install any add on','js-support-ticket')) . '.';
                                                } ?>
                                                ...
                                                <div class="jsst-addon-installer-right-addon-section" >
                                                    <?php
                                                    if(!empty($jsst_result['data'])){
                                                        $jsst_addon_availble_count = 0;
                                                        foreach ($jsst_result['data'] as $jsst_key => $jsst_value) {
                                                            if(!in_array($jsst_key, $jsst_addon_array)){
                                                                $jsst_addon_availble_count++;
                                                                $jsst_addon_slug_array = jssupportticketphplib::JSST_explode('-', $jsst_key);
                                                                $jsst_addon_image_name = $jsst_addon_slug_array[count($jsst_addon_slug_array) - 1];
                                                                $jsst_addon_slug = jssupportticketphplib::JSST_str_replace('-', '', $jsst_key);

                                                                $jsst_addon_img_path = '';
                                                                $jsst_addon_img_path = JSST_PLUGIN_URL.'includes/images/addon-images/addons/';
                                                                if($jsst_value['status'] == 1){ ?>
                                                                    <div class="jsst-addon-installer-right-addon-single" >
                                                                        <img class="jsst-addon-installer-right-addon-image" data-addon-name="<?php echo esc_attr($jsst_key); ?>" src="<?php echo esc_url($jsst_addon_img_path.esc_attr($jsst_addon_image_name).'.png');?>" />
                                                                        <div class="jsst-addon-installer-addons-btmwrp  ">
                                                                            <input type="checkbox" class="jsst-addon-installer-right-addon-single-checkbox" id="addon-<?php echo esc_attr($jsst_key); ?>" name="<?php echo esc_attr($jsst_key); ?>" value="1">
                                                                            <div class="jsst-addon-installer-right-addon-name" >
                                                                                <?php echo esc_html($jsst_value['title']);?>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <?php
                                                        ...
                                                        if($jsst_addon_availble_count == 0){ // all allowed addon are already installed
                                                            $jsst_error_message = esc_html(__('All allowed add ons are already installed','js-support-ticket')).'.';
                                                        }
                                                    }else{ // no addon returend
                                                        $jsst_error_message = esc_html(__('You are not allowed to install any add on','js-support-ticket')).'.';
                                                    }
                                                    if($jsst_error_message != ''){
                                                        $jsst_url = admin_url("admin.php?page=premiumplugin&jstlay=step1");

                                                        echo '<div class="jsst-addon-go-back-messsage-wrap">';
                                                        echo '<span>';
                                                        echo wp_kses_post($jsst_error_message);
                                                        echo '</span>';

                                                        echo '<a class="jsst-addon-go-back-link" href="'.esc_url($jsst_url).'">';
                                ...
                                <input type="hidden" name="token" value="<?php echo esc_attr($jsst_result['token']); ?>"/>
                            </form>
                        </div>
                    </div>
                </div>
