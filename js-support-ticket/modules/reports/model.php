...
class JSSTreportsModel {
    ...
    function getDepartmentReportsFE(){
        ...
        $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_staffid);
    ...
    function getStaffReports(){
        ...
        if($jsst_uid) { $jsst_query .= ' WHERE staff.uid = %d'; $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_uid); }
        $jsst_total = jssupportticket::$_db->get_var($jsst_query);
        jssupportticket::$jsst_data['total'] = $jsst_total;
        ...
        $jsst_query = "SELECT staff.photo,staff.id,staff.firstname,staff.lastname,staff.username,staff.email,user.display_name,user.user_email,user.user_nicename,
                    ...
                    JOIN `".jssupportticket::$_wpprefixforuser."js_ticket_users` AS user ON user.id = staff.uid";
        if($jsst_uid) { $jsst_query .= ' WHERE staff.uid = %d'; $jsst_query_args[] = $jsst_uid; }
    ...
    function getStaffReportsFE(){
        ...
        $jsst_nextdate = $jsst_fromdate;
        // find my depats
        $jsst_query = "SELECT dept.departmentid FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` AS dept WHERE dept.staffid = %d";
        $jsst_data = jssupportticket::$_db->get_results(jssupportticket::$_db->prepare($jsst_query, $jsst_staffid));
        $jsst_my_depts = '';
        ...
        if($jsst_my_depts)
            $jsst_dep_query = " AND (ticket.staffid = %d OR ticket.departmentid IN ($jsst_my_depts)) ";
        else
            $jsst_dep_query = " AND ( ticket.staffid = %d ) ";
        $jsst_dep_query = jssupportticket::$_db->prepare($jsst_dep_query, $jsst_staffid);
        //Query to get Data
        $jsst_query = "SELECT ticket.created FROM `" . jssupportticket::$_db->prefix . "js_ticket_tickets` AS ticket WHERE ticket.status = 1 AND (ticket.lastreply = '0000-00-00 00:00:00') AND date(ticket.created) >= %s AND date(ticket.created) <= %s";
        ...
        $jsst_query .= " WHERE (staff.id = %d OR dep.departmentid IN (SELECT dept.departmentid FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` AS dept WHERE dept.staffid = %d))";
        $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_staffid, $jsst_staffid);
        $jsst_total = jssupportticket::$_db->get_var($jsst_query);
        jssupportticket::$jsst_data['total'] = $jsst_total;
        jssupportticket::$jsst_data[1] = JSSTpagination::getPagination($jsst_total);
        // data
        $jsst_query = "SELECT DISTINCT staff.photo,staff.id,staff.firstname,staff.lastname,staff.username,staff.email,user.display_name,user.user_email,user.user_nicename,
            ...
            LEFT JOIN `".jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` AS dep ON dep.staffid = staff.id";
        $jsst_query .= " WHERE (staff.id = %d OR dep.departmentid IN (SELECT dept.departmentid FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` AS dept WHERE dept.staffid = %d))";
        $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_staffid, $jsst_staffid);
        $jsst_query .= " LIMIT " . JSSTpagination::getOffset() . ", " . JSSTpagination::getLimit();
        $jsst_agents = jssupportticket::$_db->get_results($jsst_query);
        jssupportticket::$jsst_data['staffs_report'] = $jsst_agents;
        return;
    ...
    function isValidStaffid($jsst_staffid){
        ...
        $jsst_query .= " WHERE (dep.departmentid IN (SELECT dept.departmentid FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` AS dept WHERE dept.staffid = %d))";
    ...
    function getUserReports(){
        ...
        if($jsst_uid) { $jsst_query .= " AND user.id = %d"; $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_uid); }
        $jsst_total = jssupportticket::$_db->get_var($jsst_query);
        jssupportticket::$jsst_data['total'] = $jsst_total;
        jssupportticket::$jsst_data[1] = JSSTpagination::getPagination($jsst_total);
        ...
        $jsst_query = "SELECT user.display_name,user.user_email,user.user_nicename,user.id,
                    ...
                    $jsst_query .=" NOT EXISTS (SELECT umeta_id FROM `".jssupportticket::$_wpprefixforuser."usermeta` WHERE user_id = user.id AND meta_value LIKE '%administrator%')";
        $jsst_query_args = array($jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate, $jsst_curdate, $jsst_fromdate);
        if($jsst_uid) { $jsst_query .= " AND user.id = %d"; $jsst_query_args[] = $jsst_uid; }
        $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_query_args);
        $jsst_query .= " LIMIT " . JSSTpagination::getOffset() . ", " . JSSTpagination::getLimit();
    ...
    function getStaffDetailReportByStaffId($jsst_id){
        ...
        if(in_array('timetracking', jssupportticket::$_active_addons)){
            $jsst_query = "SELECT DISTINCT(ticketid) AS ticketid
                        ...
                        WHERE staffid = %d";
            $jsst_all_tickets = jssupportticket::$_db->get_results(jssupportticket::$_db->prepare($jsst_query, $jsst_id));
            $jsst_comma = '';
        ...
        $jsst_query = "SELECT COUNT(ticket.id)
                    ...
                    WHERE ".$jsst_q_strig." AND date(ticket.created) >= %s AND date(ticket.created) <= %s ";
        $jsst_total = jssupportticket::$_db->get_var(jssupportticket::$_db->prepare($jsst_query, $jsst_curdate, $jsst_fromdate));
        jssupportticket::$jsst_data['total'] = $jsst_total;
        ...
        $jsst_query = "SELECT ticket.*,priority.priority, priority.prioritycolour,status.status AS statustitle,status.statuscolour,status.statusbgcolour ".jssupportticket::$_addon_query['select']."
                    ...
                    WHERE ".$jsst_q_strig." AND date(ticket.created) >= %s AND date(ticket.created) <= %s ";
        $jsst_query = jssupportticket::$_db->prepare($jsst_query, $jsst_curdate, $jsst_fromdate);
        $jsst_query .= " LIMIT " . JSSTpagination::getOffset() . ", " . JSSTpagination::getLimit();
    ...
    function getDepartmentDetailReportByDepartmentId($jsst_id){
        ...
        $jsst_query = "SELECT count(ticket.id)
                    FROM `".jssupportticket::$_db->prefix."js_ticket_tickets` AS ticket
                    JOIN `".jssupportticket::$_db->prefix."js_ticket_departments` AS department ON department.id = ticket.departmentid WHERE department.id = %d";
        $jsst_total = jssupportticket::$_db->get_var(jssupportticket::$_db->prepare($jsst_query, $jsst_id));
        jssupportticket::$jsst_data['total'] = $jsst_total;
        jssupportticket::$jsst_data[1] = JSSTpagination::getPagination($jsst_total);
    ...
    function getStaffDetailReportByUserId($jsst_id){
        ...
        $jsst_total = jssupportticket::$_db->get_var(jssupportticket::$_db->prepare($jsst_query, $jsst_id, $jsst_curdate, $jsst_fromdate));
        jssupportticket::$jsst_data['total'] = $jsst_total;
        jssupportticket::$jsst_data[1] = JSSTpagination::getPagination($jsst_total);
        //Tickets
        do_action('jsstFeedbackQueryStaff');
    ...
    function getStaffTimingReportById($jsst_id){
        ...
        if(in_array('timetracking', jssupportticket::$_active_addons)){
            ...
            $jsst_query .= " WHERE staffid = %d";
            $jsst_openticket = jssupportticket::$_db->get_results(jssupportticket::$_db->prepare($jsst_query, $jsst_id));
        }else{
