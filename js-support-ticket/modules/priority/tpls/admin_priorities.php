...
<div id="jsstadmin-wrapper">
    ...
    <div id="jsstadmin-data">
        ...
        <div id="jsstadmin-head">
            ...
            <a title="<?php echo esc_attr(__('Add','js-support-ticket')); ?>" class="jsstadmin-add-link button" href="?page=priority&jstlay=addpriority"><img alt = "<?php echo esc_attr(__('Add','js-support-ticket')); ?>" src="<?php echo esc_url(JSST_PLUGIN_URL); ?>includes/images/plus-icon.png" /><?php echo esc_html(__('Add Priority', 'js-support-ticket')); ?></a>
        </div>
        <div id="jsstadmin-data-wrp" class="p0 bg-n bs-n">
            <form class="js-filter-form" name="jssupportticketform" id="jssupportticketform" method="post" action="<?php echo esc_url(wp_nonce_url(admin_url("admin.php?page=priority&jstlay=priorities"),"priorities")); ?>">
                <?php echo wp_kses(JSSTformfield::text('title', jssupportticket::$jsst_data['filter']['title'], array('placeholder' => esc_html(__('Title', 'js-support-ticket')),'class' => 'js-form-input-field')), JSST_ALLOWED_TAGS); ?>
                ...
                <form class="jsstadmin-form" method="post" action="<?php echo esc_url(wp_nonce_url(admin_url("admin.php?page=jssupportticket&task=saveordering"),"save-ordering")); ?>">
                    <table id="js-support-ticket-table">
                        ...
                        foreach (jssupportticket::$jsst_data[0] AS $jsst_priority) {
                            $jsst_isdefault = ($jsst_priority->isdefault == 1) ? 'good.png' : 'close.png';
                            $jsst_ispublic = ($jsst_priority->ispublic == 1) ? 'good.png' : 'close.png';
                            $jsst_ticketoverduetype = ($jsst_priority->overduetypeid == 1) ? 'Days' : 'Hours';
                            ?>

                            <tr id="id_<?php echo esc_attr($jsst_priority->id); ?>">
                                ...
                                <td class="left"><span class="js-support-ticket-table-responsive-heading"><?php
                                        ...
                                        echo " : ";
                                        ?></span><a title="<?php echo esc_attr(__('Priority','js-support-ticket')); ?>" href="?page=priority&jstlay=addpriority&jssupportticketid=<?php echo esc_attr($jsst_priority->id); ?>"><?php echo esc_html(jssupportticket::JSST_getVarValue($jsst_priority->priority)); ?></a></td>
                                <?php if(in_array('overdue', jssupportticket::$_active_addons)){ ?>
                                    <td><span class="js-support-ticket-table-responsive-heading"><?php
                                        echo esc_html(__('Date Interval', 'js-support-ticket'));
                                        echo " : ";
                                        ?></span><?php echo esc_html(jssupportticket::JSST_getVarValue($jsst_priority->overdueinterval)); ?></td>
                                    <td><span class="js-support-ticket-table-responsive-heading"><?php
                                        echo esc_html(__('Ticket Overdue', 'js-support-ticket'));
                                        echo " : ";
                                        ?></span><?php echo esc_html(jssupportticket::JSST_getVarValue($jsst_ticketoverduetype)); ?></td>
                                <?php } ?>
                                <td><span class="js-support-ticket-table-responsive-heading"><?php
                                        echo esc_html(__('Public', 'js-support-ticket'));
                                        echo " : ";
                                        ?></span> <img alt = "<?php echo esc_attr(__('Public','js-support-ticket')); ?>" src="<?php echo esc_url(JSST_PLUGIN_URL); ?>includes/images/<?php echo esc_attr($jsst_ispublic); ?>" /></td>
                                <td><span class="js-support-ticket-table-responsive-heading"><?php
                                    echo esc_html(__('Default', 'js-support-ticket'));
                                    echo " : ";
                                    ?></span>
                                    <?php $jsst_url = '?page=priority&task=makedefault&action=jstask&priorityid='.esc_attr($jsst_priority->id);
                                    if($jsst_pagenum > 1){
                                        $jsst_url .= '&pagenum=' . $jsst_pagenum;
                                    }?><a title="<?php echo esc_attr(__('Default','js-support-ticket')); ?>" href="<?php echo esc_url(wp_nonce_url($jsst_url, 'make-default-'.$jsst_priority->id)); ?>" ><img alt = "<?php echo esc_attr(__('Default','js-support-ticket')); ?>" src="<?php echo esc_url(JSST_PLUGIN_URL); ?>includes/images/<?php echo esc_attr($jsst_isdefault); ?>" /></a></td>
                                <td><span class="js-support-ticket-table-responsive-heading"><?php
                            echo esc_html(__('Color', 'js-support-ticket'));
                            echo " : ";
                            ?></span> <span class="js-ticket-admin-prirrity-color" style="background:<?php echo esc_attr($jsst_priority->prioritycolour); ?>;color:#ffffff;"> <?php echo esc_html($jsst_priority->prioritycolour); ?></span></td>
                                <td>
                                    <a title="<?php echo esc_attr(__('Edit','js-support-ticket')); ?>" class="action-btn" href="?page=priority&jstlay=addpriority&jssupportticketid=<?php echo esc_attr($jsst_priority->id); ?>"><img alt = "<?php echo esc_attr(__('Edit','js-support-ticket')); ?>" src="<?php echo esc_url(JSST_PLUGIN_URL); ?>includes/images/edit.png" /></a>&nbsp;&nbsp;
                                    <a title="<?php echo esc_attr(__('Delete','js-support-ticket')); ?>" class="action-btn" onclick="return confirm('<?php echo esc_js(__('Are you sure you want to delete it?', 'js-support-ticket')); ?>');" href="<?php echo esc_url(wp_nonce_url('?page=priority&task=deletepriority&action=jstask&priorityid='.esc_attr($jsst_priority->id),'delete-priority-'.$jsst_priority->id));?>"><img alt = "<?php echo esc_attr(__('Delete','js-support-ticket')); ?>" src="<?php echo esc_url(JSST_PLUGIN_URL); ?>includes/images/delete.png" /></a>
                                </td>
                            </tr>
                        <?php
                        $jsst_number++;
